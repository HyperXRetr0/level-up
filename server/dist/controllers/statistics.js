...
import { calculatePercentage, getInventory, prevMonthsData, } from "../utils/features.js";
export const getDashboardStatistics = TryCatch(async (req, res, next) => {
    let stats;
    if (nodeCache.has("admin-stats"))
        ...
        const [currentMonthProducts, currentMonthUsers, currentMonthOrders, prevMonthProducts, prevMonthUsers, prevMonthOrders, productCount, userCount, allOrders, prevSixMonthOrders, productCategories, femaleUserCount, latestTransactions,] = await Promise.all([
        ...
        const deltaUsers = calculatePercentage(currentMonthUsers.length, prevMonthUsers.length);
        const deltaProducts = calculatePercentage(currentMonthProducts.length, prevMonthProducts.length);
        const deltaOrders = calculatePercentage(currentMonthOrders.length, prevMonthOrders.length);
        const currentMonthRevenue = currentMonthOrders.reduce((revenue, order) => revenue + (order.total || 0), 0);
        const prevMonthRevenue = prevMonthOrders.reduce((revenue, order) => revenue + (order.total || 0), 0);
        const deltaRevenue = calculatePercentage(currentMonthRevenue, prevMonthRevenue);
        const revenue = allOrders.reduce((revenue, order) => revenue + (order.total || 0), 0);
        // previous 6 months data for chart
        const orderMonthCount = prevMonthsData(6, prevSixMonthOrders, currentDate);
        const orderMonthlyRevenue = prevMonthsData(6, prevSixMonthOrders, currentDate, "total");
        // get category data
...
export const getPieChart = TryCatch(async (req, res, next) => {
    ...
    if (nodeCache.has("admin-pie-charts"))
        ...
        const [countProcessing, countShipped, countDelivered, productCategories, productCount, outOfStockProducts, getOrderDetails, getUserDOB, getAdminCount, getUserCount,] = await Promise.all([
        ...
        // get category data
        const productCategoryRatio = await getInventory(productCategories, productCount);
        ...
        const grossIncome = getOrderDetails.reduce((total, detail) => total + (detail.total || 0), 0);
        const totalDiscount = getOrderDetails.reduce((total, detail) => total + (detail.discount || 0), 0);
        const productionCost = getOrderDetails.reduce((total, detail) => total + (detail.shippingCharges || 0), 0);
        const totalBurn = getOrderDetails.reduce((total, detail) => total + (detail.tax || 0), 0);
        const marketingCost = grossIncome * (30 / 100); // static cost (30%)
        const netMargin = grossIncome -
...
export const getBarChart = TryCatch(async (req, res, next) => {
    ...
    if (nodeCache.has("admin-bar-charts"))
        ...
        const [prevSixMonthProducts, prevSixMonthUsers, prevTwelveMonthOrders] = await Promise.all([
            prevSixMonthProductsPromise,
            prevSixMonthUsersPromise,
            prevTwelveMonthOrdersPromise,
        ]);
        const prevSixMonthProductCount = prevMonthsData(6, prevSixMonthProducts, currentDate);
        const prevSixMonthUserCount = prevMonthsData(6, prevSixMonthUsers, currentDate);
        const prevTwelveMonthOrderCount = prevMonthsData(12, prevTwelveMonthOrders, currentDate);
        charts = {
...
export const getLineChart = TryCatch(async (req, res, next) => {
    ...
    if (nodeCache.has("admin-line-charts"))
        charts = JSON.parse(nodeCache.get("admin-line-charts"));
    else {
        const currentDate = new Date();
        const prevTwelveMonth = new Date();
        prevTwelveMonth.setMonth(prevTwelveMonth.getMonth() - 12);
        ...
        const [prevTwelveMonthProducts, prevTwelveMonthOrders, prevTwelveMonthUsers,] = await Promise.all([
            prevTwelveMonthProductsPromise,
            prevTwelveMonthOrdersPromise,
            prevTwelveMonthUsersPromise,
        ]);
        const discount = prevMonthsData(12, prevTwelveMonthOrders, currentDate, "discount");
        const revenue = prevMonthsData(12, prevTwelveMonthOrders, currentDate, "total");
        const prevTwelveMonthProductCount = prevMonthsData(12, prevTwelveMonthProducts, currentDate);
        const prevTwelveMonthUserCount = prevMonthsData(12, prevTwelveMonthUsers, currentDate);
        charts = {
